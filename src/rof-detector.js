...
export class RateOfFireDetector {
  ...
  async extractAudio(file, ffmpeg, onProgress = null) {
    ...
    // Close previous context if exists (though usually new detector = new context)
  ...
  calculateRates() {
    ...
    for (let burstIdx = 0; burstIdx < this.bursts.length; burstIdx++) {
      ...
      const medianInterval = signal.median(intervals);
      const meanInterval = signal.mean(intervals);

      // Mean Absolute Deviation (from Mean)
      const meanDev = signal.mean(intervals.map(i => Math.abs(i - meanInterval)));

      // Median Absolute Deviation (from Median)
      const medianDev = signal.median(intervals.map(i => Math.abs(i - medianInterval)));
      ...
      console.log(`Burst ${burstIdx + 1}: ${numShots} shots, ${rateRpm.toFixed(1)} RPM, MAD: ${(medianDev * 1000).toFixed(2)}ms`);
    }
  ...
  generateSummary(burstResults) {
    ...
    // Calculate overall statistics for shot intervals (consistency)
    // We can average the stats from each burst, weighted by shot count or just average them?
    // User wants "Standard Deviation" and "Mean Deviation" in summary.
    // Let's calculate the weighted average of these stats across all bursts.

    let totalWeightedStd = 0;
    let totalWeightedMeanDev = 0;
    let totalIntervalsInfo = 0;
    ...
    return {
      ...
      // Aggregate interval stats (weighted by number of intervals per burst)
      avgStdInterval: signal.mean(burstResults.map(b => b.stdInterval)), // Simple average for now
      avgMeanDeviation: signal.mean(burstResults.map(b => b.meanDeviation)), // Simple average for now
      avgMedianDeviation: signal.mean(burstResults.map(b => b.medianDeviation)) // Simple average for now
    };
  }
